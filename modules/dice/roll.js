...
export class RollFFG extends Roll {
  ...
  static CHAT_TEMPLATE = "systems/ucttg/templates/dice/roll-ffg.html";

  static TOOLTIP_TEMPLATE = "systems/ucttg/templates/dice/tooltip-ffg.html";
  ...
  async render(chatOptions = {}) {
    ...
    // Define chat data
    if (this?.data) {
      if (this.data.flags?.ucttg?.uuid) {
        const item = await fromUuid(this.data.flags.ucttg.uuid);
        if (item) {
          this.data = item;
          this.data.system = await (new ItemFFG(this.data, {validate: false}).getItemDetails());
        }
      }
      else if (this.data.flags?.ucttg?.ffgUuid) {
        const item = await fromUuid(this.data.flags.ucttg.ffgUuid);
        if (item) {
          this.data = item;
          this.data.system = await (new ItemFFG(this.data, {validate: false}).getItemDetails());
        }
    ...
    if (chatData?.data?.flags?.ucttg.hasOwnProperty('crew')) {
      chatData.data.crew = chatData.data.flags.ucttg.crew;
    }
    if (chatData.data.hasOwnProperty('data') && (chatData.data.data.adjusteditemmodifier === undefined || chatData.data.data.adjusteditemmodifier.length === 0)) {
      ...
      if (candidate_actors.length > 0) {
        if (game.settings.get("ucttg", "oldWorldCompatability")) {
        ...
        chatData.data.flags.ucttg.ffgUuid = 'Actor.' + candidate_actors[0].id + '.Item.' + chatData.data._id;
      }
    }
