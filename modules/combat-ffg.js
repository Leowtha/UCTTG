    ...
    const extraSlot = await new CombatantFFG(
{
      ...
      buttons: {
        submit: {
          ...
          callback: async (obj, event) => {
            ...
            this.setupTurns();
            game.socket.emit("system.ucttg", {event: "trackerRender", combatId: this.id});
          }
        }
      },
      default: "submit",
  ...
  async rollInitiative(ids, { formula = null, updateTurn = true, messageOptions = {} } = {}) {
    ...
    let promise = new Promise(async function (resolve, reject) {
      ...
      const content = await renderTemplate("systems/ucttg/templates/dialogs/ffg-initiative.html", {
        id,
        dicePools,
        addDicePool,
  ...
  getSlotClaims(round, slot_id) {
    const claims = this.getFlag('ucttg', 'combatClaims') || undefined;
    if (!claims) {
      return undefined;
    }
   ...
   */
  getClaims(round) {
    const combatClaims = this.getFlag('ucttg', 'combatClaims');
    if (combatClaims) {
  ...
  async handleCombatantRemoval(combatant, options, userId) {
    ...
    let action = game.settings.get("ucttg", "removeCombatantAction")
    if (action === "prompt") {
      new Dialog({
  ...
  async removeCombatantOnly(combatantId) {
    ...
    this.setupTurns();
    // emit a socket event
    game.socket.emit("system.ucttg", {event: "trackerRender", combatId: combat.id});
  }
  ...
  async claimSlot(round, slot_id, combatantId) {
    if (!game.user.isGM) {
      ...
      game.socket.emit("system.ucttg", {event: "combat", data: data});
      return;
    }
    const claims = {
      ...this.getFlag('ucttg', 'combatClaims')
    };
    if (!claims[round]) {
      claims[round] = {};
    }
    claims[round][slot_id] = combatantId;
    await this.setFlag('ucttg', 'combatClaims', claims);
  }
  ...
  async unclaimSlot(round, slot_id) {
    ...
    await this.unsetFlag('ucttg', `combatClaims.${round}.${slot_id}`);
  }
...
function _findActorForInitiative(c) {
  ...
  if (c.actor.type === "vehicle") {
    ...
    const crew = c.actor.getFlag("ucttg", "crew");
    if (crew !== undefined && crew.length > 0) {
      const initiativeCrew = crew.find((m) => m.role === "Pilot");
      if (initiativeCrew) {
  ...
  } else if (c.actor.type === "mobilesuit") {
    CONFIG.logger.debug("Actor is a mobile suit, resolving pilot for initiative.");
    const pilotId = c.actor.system.pilotId;
    if (pilotId) {
      const pilot = game.actors.get(pilotId);
      if (pilot?.system) {
        CONFIG.logger.debug(`Found pilot ${pilot.name}, merging data for initiative.`);

        // Scan MS-owned items for Characteristic modifiers.
        // Mirrors the sheet's getData() logic: pilot characteristics already
        // include pilot-item bonuses (via Foundry prepareData), so we only
        // need to add MS-owned-item Characteristic modifiers on top.
        const msCharacteristicMods = {};
        for (const item of c.actor.items) {
          ...
          for (const attr of Object.values(item.system.attributes)) {
            ...
            if (!val) continue;
            if (attr.modtype === "Characteristic") {
              msCharacteristicMods[attr.mod] = (msCharacteristicMods[attr.mod] || 0) + val;
            }
          }
        }

        // Build a merged data view. Duplicate the MS data so we don't mutate
        // the live actor's system object.
        data = foundry.utils.duplicate(c.actor.system);

        // Overlay characteristics: pilot value + MS-item bonus
        for (const charKey of Object.keys(data.characteristics)) {
          const pilotChar = pilot.system.characteristics?.[charKey];
          if (pilotChar) {
            const bonus = msCharacteristicMods[charKey] || 0;
            data.characteristics[charKey].value = pilotChar.value + bonus;
          }
        }

        // Overlay skill ranks from the pilot (matches sheet behavior).
        // Note: skill dice modifiers (boost/upgrade/etc.) are not used by
        // _buildInitiativePool — it only reads characteristic value and
        // skill.rank — so we don't need to merge those here.
        for (const skillKey of Object.keys(data.skills)) {
          const pilotSkill = pilot.system.skills?.[skillKey];
          if (pilotSkill) {
            data.skills[skillKey].rank = pilotSkill.rank;
          }
        }
      } else {
        CONFIG.logger.warn(`Mobile suit ${c.actor.name} has pilotId ${pilotId} but pilot actor not found.`);
      }
    } else {
      CONFIG.logger.warn(`Mobile suit ${c.actor.name} has no pilot assigned; initiative will use base (zero) characteristics.`);
    }
  }
...
export class CombatTrackerFFG extends CombatTracker {
  /** @override */
  get template() {
    return "systems/ucttg/templates/dialogs/combat-tracker.html";
  }
...
export function updateCombatTracker() {
  ...
  if (game.combat && game.settings.get("ucttg", "useGenericSlots")) {
