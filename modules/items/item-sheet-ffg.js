...
export class ItemSheetFFG extends ItemSheet {
  ...
  async getData(options) {
    ...
    // Derive ammo.max from the Ammo quality (includes ranks granted by attachments)
    if (["weapon", "shipweapon"].includes(this.object.type)) {
      if (!data.data.ammo) data.data.ammo = { value: 0, max: null };
      const ammoQuality = data.data.doNotSubmit?.qualities?.find(q => q.name === "Ammo");
      data.data.ammo.max = (ammoQuality && ammoQuality.totalRanks > 0)
        ? Number(ammoQuality.totalRanks)
        : null;
    }
