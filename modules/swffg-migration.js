...
export async function handleUpdate() {
  const registeredVersion = game.settings.get("ucttg", "systemMigrationVersion");
  ...
  if (registeredVersion !== runningVersion) {
    ...
    await game.settings.set("ucttg", "systemMigrationVersion", runningVersion);
  }
}
...
async function sendChanges(newVersion) {
  const template = "systems/ucttg/templates/notifications/new_version.html";
...
async function warnTheme() {
  if (game.settings.get("ucttg", "ui-uitheme") === "default") {
    const messageData = {
      user: game.user.id,
      type: CONST.CHAT_MESSAGE_TYPES.OTHER,
  ...
  for (const actor of game.actors) {
    for (const species of actor.items.filter(a => a.type === "species")) {
      for (const talent of Object.values(species.system.talents)) {
        await actor.items.find(i => i.name === talent.name)?.update({flags: {ucttg: {fromSpecies: true}}});
      }
    }
  }
}
...
async function migrateTo1_906() {
  // specializations
  let compendiums = [];
  for (const compendium of game.settings.get("ucttg", "specializationCompendiums").split(",")) {
    if (compendium.includes("ucttg.")) {
      compendiums.push(compendium.replace("ucttg.", "world."));
    } else {
      compendiums.push(compendium);
    }
  }
  game.settings.set("ucttg", "specializationCompendiums", compendiums.join(","));
  // signature abilities
  compendiums = [];
  for (const compendium of game.settings.get("ucttg", "signatureAbilityCompendiums").split(",")) {
    if (compendium.includes("ucttg.")) {
      compendiums.push(compendium.replace("ucttg.", "world."));
    } else {
      compendiums.push(compendium);
  ...
  game.settings.set("ucttg", "signatureAbilityCompendiums", compendiums.join(","));
  // force powers
  compendiums = [];
  for (const compendium of game.settings.get("ucttg", "forcePowerCompendiums").split(",")) {
    if (compendium.includes("ucttg.")) {
      compendiums.push(compendium.replace("ucttg.", "world."));
    } else {
      compendiums.push(compendium);
  ...
  game.settings.set("ucttg", "forcePowerCompendiums", compendiums.join(","));
  // talents
  compendiums = [];
  for (const compendium of game.settings.get("ucttg", "talentCompendiums").split(",")) {
    if (compendium.includes("ucttg.")) {
      compendiums.push(compendium.replace("ucttg.", "world."));
    } else {
      compendiums.push(compendium);
  ...
  game.settings.set("ucttg", "talentCompendiums", compendiums.join(","));
}
