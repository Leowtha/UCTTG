...
export class ActorSheetFFG extends ActorSheet {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["ucttg", "sheet", "actor"],
      template: "systems/ucttg/templates/actors/ffg-character-sheet.html",
      width: 710,
      height: 650,
  ...
  // Only allow character-type actors as pilots
  const validPilotTypes = ["character", "nemesis", "rival", "ace"];
  if (!validPilotTypes.includes(droppedActor.type)) {
    ui.notifications.warn("Only character-type actors can be assigned as pilots.");
    return;
  }

  // Players can only assign characters they own as pilots.
  // GMs are exempt (isOwner is always true for GMs anyway, but explicit is clearer).
  if (!game.user.isGM && !droppedActor.isOwner) {
    ui.notifications.warn(`You do not have ownership of ${droppedActor.name} and cannot assign them as a pilot.`);
    return;
  }

  await this.actor.update({ "system.pilotId": droppedActor.id });
  ui.notifications.info(`${droppedActor.name} assigned as pilot of ${this.actor.name}.`);
}
...
// Normalize all entry-list fields: legacy arrays/scalars → keyed objects
if (["character", "ace"].includes(this.actor.type)) {
  // Hangar
  data.data.hangar = this._normalizeEntryList(data.data.hangar, ["value"]);

  // Downtime sections: normalize and compute per-row "spent"
  const downtimeSections = ["downtime", "rnd", "production", "customization"];
  const downtimeTotals = { weeks: 0, months: 0, spent: 0 };

  for (const section of downtimeSections) {
    data.data[section] = this._normalizeEntryList(
      data.data[section],
      ["name", "weeks", "months"]
    );

    for (const id of Object.keys(data.data[section])) {
      const entry = data.data[section][id];
      const weeks = parseInt(entry.weeks) || 0;
      const months = parseInt(entry.months) || 0;
      const spent = weeks + months * 4;

      entry.weeks = weeks;
      entry.months = months;
      entry.spent = spent;

      downtimeTotals.weeks += weeks;
      downtimeTotals.months += months;
      downtimeTotals.spent += spent;
    }
  }

  // Compute remaining budget for visual cue
  const available = parseInt(data.data.downtimeAvailable) || 0;
  downtimeTotals.available = available;
  downtimeTotals.overspent = downtimeTotals.spent > available;

  data.downtimeTotals = downtimeTotals;
}
    ...
    if (typeof this.actor.flags?.ucttg?.config?.enableAutoSoakCalculation === "undefined") {
      autoSoakCalculation = game.settings.get("ucttg", "enableSoakCalc");
    } else {
      autoSoakCalculation = this.actor.flags?.ucttg?.config?.enableAutoSoakCalculation;
    }

    data.settings = {
      enableSoakCalculation: autoSoakCalculation,
      enableCriticalInjuries: this.actor.flags?.ucttg?.config?.enableCriticalInjuries,
    };

    // Establish sheet width and height using saved persistent values, then
    // CONFIG defaults, then the class's defaultOptions as a final fallback.
    // Never assign undefined to position.* — Foundry will skip applying any
    // dimension to the DOM and the element will balloon to natural content size.
    const cfgW = CONFIG.FFG?.sheets?.defaultWidth?.[this.actor.type];
    const cfgH = CONFIG.FFG?.sheets?.defaultHeight?.[this.actor.type];
    const fallbackW = this.constructor.defaultOptions.width ?? 710;
    const fallbackH = this.constructor.defaultOptions.height ?? 650;

    this.position.width  = this.sheetWidth  || cfgW || fallbackW;
    this.position.height = this.sheetHeight || cfgH || fallbackH;
...
if (["mobilesuit", "mobilearmor", "mobileweapon"].includes(this.actor.type) && this.actor.system.pilotId) {
  ...
  if (pilot) {
    ...
    const enrichedTalentList = [];
          for (const talent of (pilot.talentList || [])) {
        // Try both sources
        const ownedItem = pilot.items.get(talent.itemId);
        const worldItem = game.items.get(talent.itemId);

        // Activation: prefer owned (already localized), fall back to world (needs localize)
        const rawActivation = ownedItem?.system?.activation?.value 
          || worldItem?.system?.activation?.value 
          || talent.activation 
          || "Passive";

        // Description: prefer world item's short description, fall back to owned item's full description, then talent's own description
          const description = worldItem?.system?.short
          || worldItem?.system?.description
          || ownedItem?.system?.short
          || ownedItem?.system?.description
          || talent.description
          || "";

        // Activation label: try both
        const activationLabel = ownedItem?.system?.activation?.label 
          || worldItem?.system?.activation?.label 
          || "";

        enrichedTalentList.push({
          name: talent.name,
          itemId: talent.itemId,
          isRanked: talent.isRanked,
          rank: talent.rank || (talent.isRanked ? 1 : "N/A"),
          activation: game.i18n.localize(rawActivation),
    ...
    data.pilot = {
      ...
      img: pilot.img,
      id: pilot.id,
      characteristics: foundry.utils.duplicate(pilot.system.characteristics),
      skills: foundry.utils.duplicate(pilot.system.skills),
      talentList: enrichedTalentList,
    };

    // Collect ALL MS talent/item modifiers by type
    const msModifiers = {
      Characteristic: {},    // key: characteristic name, value: total bonus
      SkillBoost: {},        // key: skill name, value: total boost dice
      SkillUpgrade: {},      // key: skill name or "*", value: total upgrades
      SkillAdvantage: {},    // key: skill name, value: total auto-advantages
      StatDefence: {},       // key: "ranged" or "melee", value: total bonus
      Soak: 0,              // flat soak bonus
    };

    for (const item of this.actor.items) {
      if (item.system?.attributes) {
        for (const attr of Object.values(item.system.attributes)) {
          const val = parseInt(attr.value) || 0;
          if (!val) continue;

          switch (attr.modtype) {
            case "Characteristic":
              if (!msModifiers.Characteristic[attr.mod]) msModifiers.Characteristic[attr.mod] = 0;
              msModifiers.Characteristic[attr.mod] += val;
              break;

            case "Skill Boost":
              if (!msModifiers.SkillBoost[attr.mod]) msModifiers.SkillBoost[attr.mod] = 0;
              msModifiers.SkillBoost[attr.mod] += val;
              break;

            case "Skill Add Upgrade":
              if (!msModifiers.SkillUpgrade[attr.mod]) msModifiers.SkillUpgrade[attr.mod] = 0;
              msModifiers.SkillUpgrade[attr.mod] += val;
              break;

            case "Skill Add Advantage":
              if (!msModifiers.SkillAdvantage[attr.mod]) msModifiers.SkillAdvantage[attr.mod] = 0;
              msModifiers.SkillAdvantage[attr.mod] += val;
              break;

            case "Stat":
              if (attr.mod === "Defence-Ranged") {
                if (!msModifiers.StatDefence["ranged"]) msModifiers.StatDefence["ranged"] = 0;
                msModifiers.StatDefence["ranged"] += val;
              } else if (attr.mod === "Defence-Melee") {
                if (!msModifiers.StatDefence["melee"]) msModifiers.StatDefence["melee"] = 0;
                msModifiers.StatDefence["melee"] += val;
              }
              break;

              case "Armor Stat":
              if (attr.mod === "soak") {
                msModifiers.Soak += val;
              }
              break;
          }
        }
    ...
    // ── PILOT TALENT/ITEM MODIFIER SCAN ──
    // Scan the pilot actor's owned items (talents, abilities, gear, etc.)
    // for the same modifier types and merge them into msModifiers.
    // This ensures pilot talents that grant boost dice, upgrades,
    // advantages, defence, or soak are reflected on the MS sheet.
    for (const item of pilot.items) {
      if (item.system?.attributes) {
        for (const attr of Object.values(item.system.attributes)) {
          ...
          switch (attr.modtype) {
              case "Characteristic":
              // Skip: pilot.system.characteristics already includes
              // the pilot's own item bonuses from prepareData().
              // Only MS-owned items should add Characteristic modifiers.
              break;
    ...
    // Store modifiers on data for template access
    data.msModifiers = msModifiers;

    // Apply Characteristic modifiers: pilot base + MS bonus
    for (const [key, char] of Object.entries(pilot.system.characteristics)) {
      if (data.data.characteristics[key]) {
        const baseValue = char.value;
        const msBonus = msModifiers.Characteristic[key] || 0;
        data.data.characteristics[key].value = baseValue + msBonus;
      }
    }

        // Apply Skill data: copy pilot's fully-computed skill values
    // (which already include pilot talent bonuses from prepareData()),
    // then add MS-owned-item modifiers on top.
    for (const [key, skill] of Object.entries(pilot.system.skills)) {
      if (data.data.skills[key]) {
        data.data.skills[key].rank = skill.rank;
        data.data.skills[key].careerskill = skill.careerskill;

        // Copy the pilot's computed dice modifier fields.
        // These already include bonuses from pilot-owned talents/items
        // because Foundry's prepareData() processed them on the pilot actor.
        data.data.skills[key].boost = (skill.boost || 0);
        data.data.skills[key].setback = (skill.setback || 0);
        data.data.skills[key].force = (skill.force || 0);
        data.data.skills[key].advantage = (skill.advantage || 0);
        data.data.skills[key].dark = (skill.dark || 0);
        data.data.skills[key].light = (skill.light || 0);
        data.data.skills[key].failure = (skill.failure || 0);
        data.data.skills[key].threat = (skill.threat || 0);
        data.data.skills[key].success = (skill.success || 0);
        data.data.skills[key].triumph = (skill.triumph || 0);
        data.data.skills[key].despair = (skill.despair || 0);
        data.data.skills[key].upgrades = (skill.upgrades || 0);
        data.data.skills[key].remsetback = (skill.remsetback || 0);

        // Copy source tracking arrays for tooltip display
        data.data.skills[key].ranksource = skill.ranksource || [];
        data.data.skills[key].boostsource = skill.boostsource || [];
        data.data.skills[key].setbacksource = skill.setbacksource || [];
        data.data.skills[key].remsetbacksource = skill.remsetbacksource || [];
        data.data.skills[key].advantagesource = skill.advantagesource || [];
        data.data.skills[key].darksource = skill.darksource || [];
        data.data.skills[key].lightsource = skill.lightsource || [];
        data.data.skills[key].failuresource = skill.failuresource || [];
        data.data.skills[key].threatsource = skill.threatsource || [];
        data.data.skills[key].successsource = skill.successsource || [];
        data.data.skills[key].triumphsource = skill.triumphsource || [];
        data.data.skills[key].despairsource = skill.despairsource || [];
        data.data.skills[key].upgradessource = skill.upgradessource || [];

        // Add MS-owned-item modifiers on top of the pilot's values
        const msBoost = (msModifiers.SkillBoost[key] || 0);
        const msUpgrade = (msModifiers.SkillUpgrade[key] || 0) + (msModifiers.SkillUpgrade["*"] || 0);
        const msAdvantage = (msModifiers.SkillAdvantage[key] || 0);

        data.data.skills[key].boost += msBoost;
        data.data.skills[key].upgrades += msUpgrade;
        data.data.skills[key].advantage += msAdvantage;

        // Keep display-only fields for template reference
        data.data.skills[key].msBoost = msBoost;
        data.data.skills[key].msUpgrade = msUpgrade;
        data.data.skills[key].msAdvantage = msAdvantage;
      }
    }

        // Reset defence before recalculating
    if (data.data.stats?.defence) {
      data.data.stats.defence.melee = 0;
      data.data.stats.defence.ranged = 0;
    }

    // Apply Defence modifiers to MS stats
    if (data.data.stats?.defence) {
      data.data.stats.defence.ranged += (msModifiers.StatDefence["ranged"] || 0);
      data.data.stats.defence.melee += (msModifiers.StatDefence["melee"] || 0);
    }

    // Apply Defence from equipped armour items directly
    const equippedArmour = data.items.filter(i =>
      i.type === "armour" && i.system?.equippable?.equipped === true
    );
    for (const armour of equippedArmour) {
      const armourDef = parseInt(armour.system?.defence?.value, 10) || 0;
      if (armourDef > 0) {
        data.data.stats.defence.melee += armourDef;
        data.data.stats.defence.ranged += armourDef;
      }
    }

    // Apply Soak modifier to MS stats (talents/abilities only, not gear)
    let talentSoakBonus = 0;
    for (const item of this.actor.items) {
      if (["talent", "ability"].includes(item.type) && item.system?.attributes) {
        for (const attr of Object.values(item.system.attributes)) {
          if (attr.modtype === "Armor Stat" && attr.mod === "soak") {
            talentSoakBonus += (parseInt(attr.value) || 0);
          }
        }
      }
    }
    if (data.data.stats?.soak) {
      data.data.stats.soak.value += talentSoakBonus;
    }
  ...
  for (const key in data.data.skills) {
    data.data.skills[key].rank = 0;
    data.data.skills[key].careerskill = false;
    data.data.skills[key].msBoost = 0;
    data.data.skills[key].msUpgrade = 0;
    data.data.skills[key].msAdvantage = 0;
  }

  for (const key in data.data.characteristics) {
    data.data.characteristics[key].value = 0;
  }

  data.msModifiers = null;
}
      ...
      case "vehicle":
        ...
        const crew = this.actor.getFlag('ucttg', 'crew');
        if (crew) {
    ...
    if (this.actor.flags?.ucttg?.xpLog) {
      data.xpLog = this.actor.flags.ucttg.xpLog.join("<br>");
    }
  ...
  activateListeners(html) {
    ...
    html.find("input.hangar-new").on("change", async (event) => {
      event.preventDefault();
      const value = (event.currentTarget.value || "").trim();
      if (!value) return;
      const newId = foundry.utils.randomID();
      // Clear the synthetic input so _updateObject doesn't see leftover state
      event.currentTarget.value = "";
      await this.object.update({
        [`system.hangar.${newId}`]: { value: value }
      });
    });

    // ── Downtime: remove entry from any section ──
    html.find(".remove-downtime").on("click", async (event) => {
      event.preventDefault();
      const section = event.currentTarget.dataset.section;
      const id = event.currentTarget.dataset.id;
      await this.object.update({
        [`system.${section}`]: { [`-=${id}`]: null }
      });
    });

    // ── Downtime: capture trailing empty row on change in any section ──
    html.find("input.downtime-new").on("change", async (event) => {
      event.preventDefault();
      ...
      const nameInput = row.querySelector('input.downtime-new[data-field="name"]');
      const weeksInput = row.querySelector('input.downtime-new[data-field="weeks"]');
      const monthsInput = row.querySelector('input.downtime-new[data-field="months"]');

      const name = (nameInput?.value || "").trim();
      const weeks = parseInt(weeksInput?.value) || 0;
      const months = parseInt(monthsInput?.value) || 0;

      if (!name && weeks === 0 && months === 0) return;
    ...
    Hooks.on("preCreateItem", (item, createData, options, userId) => {
      ...
      if (item.isEmbedded && item.parent.documentName === "Actor")
      {
        ...
        if (item.type === "species" || item.type === "career") {
          if (["character", "nemesis", "rival", "ace"].includes(actor.type)) {
            const itemToDelete = actor.items.filter((i) => (i.type === item.type) && (i.id !== item.id));
            itemToDelete.forEach((i) => {
                actor.items.get(i.id).delete();
            });
          }
          else if (actor.type === "minion" || actor.type === "mobilesuit") {
            ui.notifications.warn(`Item type '${item.type}' cannot be added to '${actor.type}' actor types.`);
            return false;
          }
        }

        // Critical Damage can only be added to "vehicle" actors and Critical Injury can only be added to character-type or mobilesuit actors.
    ...
    if (this.actor.type === "character" || this.actor.type === "ace") {
      this.sheetoptions = new ActorOptions(this, html);
      this.sheetoptions.register("enableAutoSoakCalculation", {
        name: game.i18n.localize("SWFFG.EnableSoakCalc"),
        hint: game.i18n.localize("SWFFG.EnableSoakCalcHint"),
      ...
      this.sheetoptions.register("medicalItemName", {
        ...
        type: "String",
        default: game.settings.get("ucttg", "medItemName"),
      });
  ...
  if (["mobilesuit", "mobilearmor", "mobileweapon"].includes(this.actor.type)) {
    ...
    html.find(".medical").click(async (ev) => {
      ...
      let updateData = {};
      let newUses = 0;
      const item_name = this.object?.flags?.ucttg?.config?.medicalItemName || game.i18n.localize("SWFFG.DefaultMedicalItemName");
      let msg_content;
    ...
    html.find(".resetMedical").click(async (ev) => {
      if (game.settings.get("ucttg", "HealingItemAction") === '0') {
          // prompt
      ...
      } else if (game.settings.get("ucttg", "HealingItemAction") === '1') {
        // rest
        let updateData = {};
        foundry.utils.setProperty(updateData, `system.stats.medical.uses`, 0);
        foundry.utils.setProperty(updateData, `system.stats.strain.value`, 0);
        foundry.utils.setProperty(updateData, `system.stats.wounds.value`, Math.max(0, this.object.system.stats.wounds.value - 1));
        this.object.update(updateData);
      } else if (game.settings.get("ucttg", "HealingItemAction") === '2') {
        // reset
        let updateData = {};
        foundry.utils.setProperty(updateData, `system.stats.medical.uses`, 0);
        this.object.update(updateData);
    ...
    html.find(".crew-edit").click(async (ev) => {
      ...
      const registeredRoles = game.settings.get('ucttg', 'arrayCrewRoles');
      const actor = this.actor;
      const vehicleRoles = actor.getFlag('ucttg', 'crew');
      ...
      const content = await renderTemplate(
        "systems/ucttg/templates/dialogs/ffg-crew-change.html",
        {
          actor: crew_member,
    ...
    html.find(".item-info").click((ev) => {
      ...
      new Dialog(
        ...
        {
          classes: ["dialog", "ucttg"],
          template: "systems/ucttg/templates/actors/dialogs/ffg-talent-selector.html",
        }
      ).render(true);
    });
    ...
    html.find(".roll-button-crew").children().on("click", async (event) => {
      ...
      const registeredRoles = await game.settings.get('ucttg', 'arrayCrewRoles');
    ...
    html.find(".roll-button-weapon").on("click", async (event) => {
      ...
      const crew = await ship.getFlag("ucttg", "crew");
      const skillRoles = game.settings.get("ucttg", "arrayCrewRoles").filter(role => role.role_skill === weaponSkill);
    ...
    html.find(".force-conflict .enable-dice-pool").on("click", async (event) => {
      event.preventDefault();
      await this.actor.setFlag('ucttg', 'config', {enableForcePool: false});
    });
  ...
  async _itemDetailsToChat(itemId) {
    ...
    const template = "systems/ucttg/templates/chat/item-card.html";
  ...
  async _forcePowerDetailsToChat(itemId, desc, name) {
    ...
    const template = "systems/ucttg/templates/chat/force-power-card.html";
    const html = await renderTemplate(template, { itemDetails, item });
  ...
  _onChangeSkillCharacteristic(a) {
    ...
    new Dialog(
      ...
      {
        classes: ["dialog", "ucttg"],
        template: "systems/ucttg/templates/actors/dialogs/ffg-skill-characteristic-selector.html",
      }
    ).render(true);
  }
  ...
  _onCreateSkill(a) {
    ...
    new Dialog(
          ...
          one: {
            ...
            callback: (html) => {
              ...
              let newSkill = {
                ...
                groupskill: false,
                label: name,
                max: game.settings.get("ucttg", "maxSkill"),
                rank: 0,
                type: group,
                custom: true,
                nontheme: true,
      ...
      {
        classes: ["dialog", "ucttg"],
        template: "systems/ucttg/templates/actors/dialogs/ffg-skill-new.html",
      }
    ).render(true);
  }
...
async _updateObject(event, formData) {
  ...
  // Drop the synthetic trailing hangar input — it's handled by its own
  // change listener and must not leak into the actor's stored data.
  if ("hangar-new-entry" in formData) {
    delete formData["hangar-new-entry"];
  }

  // Drop synthetic trailing downtime inputs (any key starting with "downtime-new-")
  // — handled by their own change listener and must not leak into stored data.
  for (const key of Object.keys(formData)) {
    if (key.startsWith("downtime-new-")) {
      delete formData[key];
    }
  }
  ...
  async _updateSpecialization(data) {
    ...
    if (this.actor.flags.ucttg === undefined) {
        this.actor.flags.ucttg = {};
    }
    this.actor.flags.ucttg.loaded = true;
...
/**
 * Normalize a field that may be a legacy array, scalar, legacy-flat-object,
 * or undefined into an object keyed by random IDs. Each entry will only
 * retain the listed fields.
 * @param {*} value - The raw field value
 * @param {string[]} validFields - Names of fields to preserve on each entry
 * @returns {Object}
 */
_normalizeEntryList(value, validFields) {
  // Legacy array form: [{ ... }, { ... }] → keyed object
  if (Array.isArray(value)) {
    const normalized = {};
    for (const entry of value) {
      if (entry && typeof entry === "object") {
        const hasContent = validFields.some(
          f => entry[f] !== undefined && entry[f] !== "" && entry[f] !== 0
        );
        if (!hasContent) continue;

        const cleaned = {};
        for (const f of validFields) {
          if (entry[f] !== undefined) cleaned[f] = entry[f];
        }
        normalized[foundry.utils.randomID()] = cleaned;
      }
    }
    return normalized;
  }

  // Null/undefined/scalar → empty object
  if (!value || typeof value !== "object") {
    return {};
  }

  // Object form — but it could be either:
  //   (a) a proper keyed-entries object: { abc123: { name, weeks, months }, ... }
  //   (b) a legacy flat-fields object:    { days: 0, weeks: 0, totalMonths: 0 }
  // Detect (b) by checking whether any value is NOT an object.
  // If we find scalar values, treat the whole thing as legacy and discard.
  const values = Object.values(value);
  const looksLegacy = values.length > 0 && values.some(
  ...
  // Otherwise it's already a proper keyed-entries object — return as-is.
  return value;
}
  /**
...
_createSkillColumns(data) {
  // Build an ordered list of skill type groups, each with a header + sorted skills
  const groups = [];
  ...
  data.data.skilltypes.forEach((type) => {
    ...
    if (game.settings.get("ucttg", "skillSorting")) {
      sortFunction = (a, b) => {
    ...
    // Each group takes 1 header row + N skill rows
    groups.push({
      type,
      skills,
      rowCount: 1 + skills.length,
    });
  });

  // Find the best split point between groups to balance the two columns
  const totalRows = groups.reduce((sum, g) => sum + g.rowCount, 0);
  const halfRows = Math.ceil(totalRows / 2.0);

  let bestSplit = 1; // split after at least the first group
  let bestDiff = Infinity;
  let runningTotal = 0;

  for (let i = 0; i < groups.length; i++) {
    runningTotal += groups[i].rowCount;
    const diff = Math.abs(runningTotal - (totalRows - runningTotal));
    if (i < groups.length - 1 && diff < bestDiff) {
      bestDiff = diff;
      bestSplit = i + 1; // split after group index i
    }
  }
  ...
  groups.forEach((group, i) => {
    const col = i < bestSplit ? 0 : 1;
    cols[col].push({ id: "header", ...group.type });
    group.skills.forEach((s) => {
      cols[col].push({ name: s, ...data.data.skills[s] });
    });
  });
  ...
  async _buyCore(event) {
    ...
    const template = "systems/ucttg/templates/dialogs/ffg-confirm-purchase.html";
    let content;
    const availableXP = this.object.system.experience.available;
    const totalXP = this.object.system.experience.total;
    let itemType;
    ...
    if (action === "specialization") {
      ...
      const sources = game.settings.get("ucttg", "specializationCompendiums").split(",");
      let outCareer = [];
      ...
      const sources = game.settings.get("ucttg", "signatureAbilityCompendiums").split(",");
      ...
      const sources = game.settings.get("ucttg", "forcePowerCompendiums").split(",");
      let selectableItems = [];
      ...
      const sources = game.settings.get("ucttg", "talentCompendiums").split(",");
      let selectableItems = [];
  ...
  _buyCharacteristicRank(characteristic) {
    ...
    if (characteristicValue >= game.settings.get("ucttg", "maxAttribute")) {
