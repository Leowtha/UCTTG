export default class ActorOptions {
  ...
  handler(event) {
    ...
    new Dialog(
      ...
      },
      {
        ...
        template: "systems/ucttg/templates/dialogs/ffg-sheet-options.html",
      }
    ).render(true);
  }
  ...
  async register(optionName, options) {
    ...
    if (typeof this.data.object.flags?.ucttg?.config == "undefined") {
      await this.data.object.setFlag("ucttg", "config", {});
    }

    if (typeof this.data.object.flags?.ucttg?.config[optionName] !== "undefined") {
      this.options[optionName].value = this.data.object.flags?.ucttg?.config[optionName];
    } else {
