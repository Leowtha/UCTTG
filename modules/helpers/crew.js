...
export function deregister_crew(vehicle_actor, crew_member, crew_role) {
    ...
    const flag_data = vehicle_actor.getFlag('ucttg', 'crew');
    let new_flag_data = [];
    ...
    if (new_flag_data.length === 0) {
      ...
      vehicle_actor.unsetFlag('ucttg', 'crew');
    } else {
      vehicle_actor.setFlag('ucttg', 'crew', new_flag_data);
    }
}
...
export async function selectRoles(vehicle, crew_member_id) {
  const crew_member = game.actors.get(crew_member_id);
  const registeredRoles = game.settings.get('ucttg', 'arrayCrewRoles');
  const vehicleRoles = vehicle.getFlag('ucttg', 'crew') || [];
